...
class Med_Tagger(object):
    ...
    def __init__(self):
        ...
        """
        self._tagger = subprocess.Popen('docker run -a stdin -a stdout -i med-tagger:1.0.0', shell=True,
    ...
    def get_results(self, text):
        ...
        while True:        
                while True:
                    output = self._nbsr.readline(0.1) # TODO Se puede mejorar eficiencia reduciendolo, pero puede producir errores
                    # 0.1 secs to let the shell output the result
                    if not output:
                        stop_it = stop_it + 1
                        break
                    results.append(tuple(output.decode('utf-8').split(' ')))
                if len(results) > 0 : # Quitado el numero de intentos
                        break
    ...
    def __del__(self):
        ...
        """
        self._tagger.stdin.close() # Added
        # self._tagger.stdout.close()
        self._tagger.kill()

        self._tagger.wait() # Added
