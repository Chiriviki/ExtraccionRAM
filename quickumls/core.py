...
class QuickUMLS(object):
    ...
    def __init__(
        ...
        Args:
            ...
            similarity_name (str, optional): One of "dice", "jaccard", "cosine".
        ...
        valid_similarities = {'dice', 'jaccard', 'cosine'}
        ...
        # Check whether adr_tagger_data is installed with lowercase flag and quickumls initiated with keeping uppercase words
        ...
        #                      'characters. Please reinstall adr_tagger_data without --lowercase or run quickumls without'
        #                    '--keep_uppercase.')
        ...
        try:
            self._stopwords = set(nltk.corpus.stopwords.words('spanish')) # Solo español
        except LookupError:
        ...
        self._stopwords = set(nltk.corpus.stopwords.words('spanish'))
        ...
        if spacy_component:
            ...
            #    )
            raise OSError("Esta distribución solo es válida para su uso como componente spacy")
        ...
        self.ss_db = toolbox.Simstring(terms_path, similarity_name, threshold)
        self.cuisem_db = toolbox.TermsDB(terms_path)
    ...
    def _is_valid_start_token(self, tok):
        return not(
            tok.like_num or
            (self._is_stop_term(tok) ) or # and tok.lemma_ not in self.negations
            tok.pos_ == 'ADP' or tok.pos_ == 'DET' or tok.pos_ == 'CONJ'
        )
